...
namespace Kernel.Primitives
{
	public static class Primitives
	{
		...
		public static class Applicatives
		{
			...
			public static Object Apply(Applicative a, Pair p, Object[] objects)
			{
				if (objects.Length > 1)
				...
				return Environment.Current.Evaluate(new Pair(a.combiner, p));
			}
			...
			#region Equality
#warning Need to think about these methods
			[Primitive("eq?", 0, true)]
			public static Boolean Eq(List objects)
			=> (Boolean)(!objects.Any() || objects.All(obj => ReferenceEquals(obj, objects[0])));
		...
		public static class Operatives
		{
			...
			public static Operative Vau(Environment env, Object formals, Object eformal, Object[] expr)
			{
				...
				return new Operative(Environment.Current.Copy() as Environment,
									 ...
									 eformal,
									 Applicatives.CopyEvaluationStructureImmutable(expr));

			}

			[Primitive("$lambda", 3)]
			[TypeAssertion(0, typeof(Environment))]
			public static Combiner Lambda(Environment env, Object formals, Object expr)
			=> Applicatives.Wrap(Vau(Environment.Current, formals, Ignore.Instance, expr));


			[Primitive("$sequence", 1, true)]
			[TypeAssertion(0, typeof(Environment))]
			public static Object Sequence(Environment env, Object[] objects)
			{
			...
			public static Object Cond(Environment env, Pair[] objects)
			{
				foreach (Pair p in objects)
				{
					...
					if (condition)
						return Sequence(env, (p.Cdr as Pair).ToArray());
				}
				return Inert.Instance;
			}
